...
3. **Essentials only / All properties** (`inspector.setMode`), remembered once chosen. **All properties offers every value of the catalogue for every control**: the generated keywords and units Chrome, Firefox and Safari all support (`list: "generated"`), so all four flex directions, every text-align value, every justify and align value including space-between, space-around and space-evenly, and every flex-wrap value. A declared subset of a property's values (with its reason in `properties.json`) is offered only in Essentials only (the door's `offers.essentials`) and in the quick panel (`offers.list`); `manifest:check` rule `all-properties` fails on an inspector field whose All properties list is a subset.
