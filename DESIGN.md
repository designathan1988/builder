...
| `styles` | left, Styles view | The project classes, each with its use count and Rename/Delete controls; a class's style values are edited through the selector bar. The variables are the project's design tokens, grouped by type (Colours, Sizes, Fonts): 1 New variable · 2 variable name field · 3 variable value field · 4 Delete the variable. |
...
| `layers-row` | every Layers row | 1 click (select) · 2 Shift+click (add) · 3 Ctrl+click (toggle) · 4 caret · 5 colour dot · 6 name (double-click renames) · 7 name field · 8 Hide on the published page · 9 Lock (8 and 9 appear on the row under the pointer, so a selected row keeps its whole name; the row of a hidden element is dimmed and keeps its Hide shown, pressed; the row of a locked element keeps its Lock shown, pressed) · 10 pick as the interaction target · 11 secondary click (context menu). Beside the name, the details "What each row shows" chooses (HTML tag, #id, .classes, attributes), in the `layers-row-details` component, drawn only on a row that has one of them. |
