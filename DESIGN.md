...
The Select parts editor presents each Option's Text and Value beside its move and remove controls. Summary and Legend use the same Text field while retaining child elements. A Link tag edit warns which link-only fields will be removed before Enter commits it; Settings then follows the new tag.
The page body's title attribute is labelled Tooltip, distinct from the Page title in SEO. Type-specific attributes appear only on their applicable element kinds; an untitled Embedded frame is called out by Checks.
...
| `dock-checks` | bottom | 1 an issue row (selects its element). An iframe without Title produces an Accessibility issue; entering Title clears it. Additional categories from `manifest/checks.json` arrive with their features. |
