...
//   - a door of a built command that a user can run (drawn enabled, or a shortcut) is run by no test.
...
interface Command {
  ...
  readonly entryPoints: readonly { readonly id: string; readonly kind: string }[];
}
...
test('every working command is proven by a browser test, and no door looks usable without a command', async ({ page }) => {
  ...
  for (const c of COMMANDS.filter((c) => BUILT.has(c.id))) {
    ...
    const reachable = c.entryPoints.filter((d) => d.kind === 'shortcut' || drawn.get(`${c.id}#${d.id}`) === true).map((d) => `${c.id}#${d.id}`);
    if (cannotRunYet) {
      // every door drawn, and every shortcut, is shown to be unavailable
      const doors = c.entryPoints.filter((d) => d.kind === 'shortcut' || drawn.has(`${c.id}#${d.id}`)).map((d) => `${c.id}#${d.id}`);
