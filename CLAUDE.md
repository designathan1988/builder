...
- A commit on `main` takes only a working tree `npm run check` validated. A commit on any other branch (`integration`, `feature/`, `wip/`) takes none: it saves the work, and the tests run at the end of each block (the user's decision). Save every finished feature, and at least every hour: `git add -A`, a commit on `integration`, `git push origin integration`, then `git status` clean; never leave a merge open. A push to main takes only a commit that `npm run verify:fast` and `npm run e2e` passed on: that is the checkpoint. The guard judges the repository the git command runs in (`git -C`, else the hook's cwd), never the main folder.
