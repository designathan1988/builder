...
// every selected node and the primary's label (its name and its exported tag, so the page root reads "body"), and
...
const EMPTY: Layout = { selected: [], hovered: null, label: null };
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (iframe && origin) {
        ...
        const next: Layout = { selected, hovered: hoveredBox, label: placed };
