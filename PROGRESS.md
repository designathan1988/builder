...
2. (builder, item 1i) Keywords of edited properties that css-compat.json supports only through their property's entry (BCD has no entry of their own; 3,529 of them, most of them named colours) and that may do nothing or be valid only inside a longer value. Not excluded; the user decides each:
   - display `run-in` (Blink removed run-in in 2013, Firefox never shipped it, WebKit removed it) and `flow` alone (a multi-keyword display value; `flow` alone is not clearly implemented).
   - break-before and break-after `all` (breaks through every fragmentation context; no browser documents it).
   - justify-items and justify-self `first`, `last`, `baseline`, `safe`, `unsafe`, `self-start`, `self-end`, `flex-start`, `flex-end`; align-content, align-items and align-self `first` and `last` (valid only as `first baseline` / `last baseline`).
   - font-feature-settings `on` and `off`, scroll-snap-type `mandatory` and `proximity`, rotate `x`, `y` and `z`, grid-*-start and -end `span`: valid only after a tag, an axis, an angle or a line number, never alone.
   - list-style-type offers named colours and `block-start` / `block-end`, and filter and list-style-type offer the deprecated system colours (activeborder, activecaption, appworkspace, background, buttonhighlight, buttonshadow, inactiveborder…): these come from nested syntaxes the generator walks, and a colour is no list marker.
