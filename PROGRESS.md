...
- 7.1d committed scenario correction: `settings-grammar-refuses-paragraph-in-label` selected Input through Explorer but then tried the Insert tile while Explorer remained open. It failed before the action because the tile was hidden (`e2e-scenarios-7-1d-wip.log`). Retired it for `settings-grammar-refuses-paragraph-after-layer-selection`, which reopens Insert before the same refused action and keeps the same document, selection, history, feedback and refusal assertions; the corrected scenario passes (`e2e-scenario-7-1d-label-corrected.log`).
- 7.1d browser correction: a visible headed Chrome was briefly opened by the former `.cache/scratch/uso-lib.mjs` before the user forbade computer browser windows. Its `finally` closed the window; the library now uses `headless: true` only for ZIP inspection. Real-use verification moved to the Codex in-app browser at `.cache/logs/uso-7.1d-20260926-1715/`; no further computer browser windows are permitted.
- 7.1d proof: four scenarios and two focused browser specs pass; targeted feature-off runs for selected, Summary and tag fail on document assertions, the Label rule disabled fails on a refusal assertion and passes restored, and Option Text/Value disabled separately fail the UI document assertion. Logs: `e2e-scenarios-7-1d-final.log`, `e2e-ui-7-1d.log`, `tooth-7-1d-*.log`. Desktop 100% and Phone Fit in-app browser measured refusal, tag warning/Undo/export, Summary text and Option text/value after reload; console 0 (`.cache/logs/uso-7.1d-20260926-1715/`). Census initially found six unbuilt page fields usable because the hidden form submit button stayed enabled; disabling that button while its door is unavailable made the targeted census pass (`e2e-census-7-1d-repaired.log`).
- 7.1e1 proof: the new `settings-type-fields` feature owns 17 type-specific attribute doors plus View > Checks and issue selection; its 19 scenarios pass. Targeted feature-off runs make all 17 fields fail document assertions, issue selection fail a selection assertion, and View > Checks fail a feedback assertion. The Tooltip label and iframe issue owner separately fail focused UI assertions when disabled and pass restored. Typecheck, lint, manifest check, one targeted unit test, two focused UI tests and census pass; in-app Desktop 100%/Phone Fit evidence and a headless ZIP inspection are in `.cache/logs/uso-7.1e-20260926-1808/`. The original `settings-audit` scenarios remain unchanged.
- 7.1e1 committed scenario correction: `settings-type-fields-list-type-is-stored` expected computed `list-style-type: decimal` after choosing HTML ordered-list `type=A`, but Chrome computes `upper-alpha` (`e2e-scenarios-7-1e1-wip.log`). Retired it for `settings-type-fields-list-type-renders-upper-alpha`, keeping the same document, history, persistence and export assertions while checking the correct rendered style.
