...
2. (builder, item 1i, rewritten after the auditor's NOTE on 454c764) The earlier list of suspects named values the generated lists do not offer alone (scroll-snap-type mandatory/proximity, rotate x/y/z, font-feature-settings on/off, list-style-type colours); it is replaced by the Chrome test above, which settles what Chrome's parser accepts. What it cannot settle: a value Chrome parses but does not act on (as `blink`); none is known beyond the region and blink entries.
3. (auditor, a92d782, NOTE) With undoCommand, redoCommand or setWorkbenchState replaced by a no-op only unit tests fail: no Chrome test runs Undo, Redo or the dock strip's toggle and maximize through their doors. Undo and Redo have nothing to undo until a group 02 command changes the document; their scenarios (undo-redo) cover them then.
4. (auditor, a92d782, NOTE) The Styles view (activity bar Styles, View › Variables) shows only its headings, with no "not available yet"; the dock tabs and the Insert tiles do say it.
