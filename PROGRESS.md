...
20. (auditor, 9cbe45e, NOTE) builder/use-ports misses performance.now(), crypto.getRandomValues() and destructured reads (const { now } = Date). To fix after item 6 lands: the lint plugin is item 6's file while a helper builds that item.
