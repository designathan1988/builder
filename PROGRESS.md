...
- 7.1e1 contract: type-specific attribute doors currently belong to unbuilt `settings-audit`; the new `settings-type-fields` feature owns them and copies their existing terminal scenarios under new IDs without editing the committed originals. The body `title` label becomes Tooltip, distinct from Page title. The iframe title warning in Checks is part of this contract and needs implementation before delivery.
