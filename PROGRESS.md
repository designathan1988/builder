...
8. (auditor, 1404fdd, NOTE) `parseFile` in `src/manifest/check.ts` caches by the parsed object alone, not by the pair of schema and object; a frozen object parsed with a second schema would get the first schema's result. Today each generated file has one schema.
