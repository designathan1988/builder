...
| i18n runtime | `src/i18n/index.ts` | `t(key, params)` over the en (default) and pt-BR catalogues, the active locale. | UI text written in code (lint rule `builder/no-literal-ui-string`). |
...
| Door rendering | `src/editor/doors/Door.tsx` | Draws one door as the control its kind and face call for, disabled with "not available yet" when its command has the marker. | Controls that are not doors (except `data-local` ones DESIGN.md lists). |
...
| The shell regions | `src/editor/shell/Shell.tsx` | The window grid and every region of DESIGN.md: top bar, activity bar and sidebar views, file tabs, canvas toolbar, canvas frame with breakpoint tabs and rulers, inspector, dock, status bar. | Drawing a control a door does not declare. |
