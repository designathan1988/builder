...
3. **Ctrl duplicates while dragging** (the ghost reads `Copy of …`) and also turns snapping off. Required: modifiers come from the one keymap table; a free drag with Alt suspends snapping (features.json `snap-while-moving`); duplication-by-drag, if kept, uses a modifier that is not also a snap switch.
