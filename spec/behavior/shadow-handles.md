...
2. **Silent defaults:** starting on an element with no shadow invents `rgba(15, 23, 42, 0.24)` with blur 12. Required: the shadow modes are disabled (with the reason) when the element has no shadow (features.json starts from "a Container with a box shadow").
3. **No status message.** Required: live value during the drag and `Shadow set to <value>` at the end.
4. **Text shadows have no canvas handles.** Required: on a text element with a text shadow, Shadow offset and Shadow blur edit its first text shadow (X, Y and blur) and write `text-shadow`, each drag one undo step (features.json `shadow-handles`).
