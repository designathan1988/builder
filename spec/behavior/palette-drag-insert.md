...
3. **Wrappers are created silently by the drop** when the type needs a parent (`✚ <ul> will be created here`), which contradicts the nesting rules the new app follows (features.json `nesting-grammar`: such attempts are refused with a message). Required: the drop is refused with the nesting message and no insertion line; nothing is inserted on release.
