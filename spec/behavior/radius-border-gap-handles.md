...
4. **Gap mode writes `row-gap` in a column flex and a direct handle offers `column-gap` there too,** where it has no visible effect. Required: Gap writes `gap`; Row gap and Column gap write their own property (features.json: "write gap, row-gap or column-gap"); options that cannot change the layout (column-gap in a single-column flex) are disabled with the reason.
