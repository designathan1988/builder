...
5. **Gradient changes have no single writer that other controls reuse.** Required: every gradient change is written by one command, which the quick panel's Fill (the same fill editor) also uses (features.json `gradient-editor`, `quick-panel`).
