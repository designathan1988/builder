...
3. **A wrong language silently becomes `en`** at render and export, so the page ends up declared English without anyone having chosen it. Required: a language that is not shaped like a language tag (letters, then optional `-` subtags of letters and digits, BCP 47) is refused, the status bar says which value and which setting (`status.page.settingInvalid`), the document keeps its value, nothing is recorded, and the field shows the document's value again. The same holds for a direction other than `ltr`, `rtl` or `auto` (the keywords `elements.json` gives `pageDirection`; Pager drops `auto`).
